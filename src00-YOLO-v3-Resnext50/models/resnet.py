...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.bn1(self.conv1(x))) # torch.Size([2, 64, 128, 128])
        x = self.maxpool(x) # torch.Size([2, 64, 64, 64])

        x1 = self.layer1(x) # torch.Size([2, 256, 64, 64])
        x2 = self.layer2(x1) # torch.Size([2, 512, 32, 32])
        x3 = self.layer3(x2) # torch.Size([2, 1024, 16, 16])
        x4 = self.layer4(x3) # torch.Size([2, 2048, 8, 8])
